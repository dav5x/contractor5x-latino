/*
 * DESIGN: Industrial Brutalism — Dark sticky nav, orange CTA, mega menu for Solutions, Learn dropdown
 */
import { useState, useEffect, useRef } from "react";
import { Menu, X, ChevronDown, Zap, Camera, Bot, Globe, RefreshCw, Star, BookOpen, ArrowRight } from "lucide-react";
        ...
        <div className="flex items-center justify-between h-16 md:h-20">
          ...
          <nav className="hidden md:flex items-center gap-6">
            ...
            <div className="relative" ref={solutionsRef}>
              <button
                onClick={() => { setSolutionsOpen(!solutionsOpen); setLearnOpen(false); }}
                className={`flex items-center gap-1.5 text-sm font-medium transition-colors duration-200 orange-underline ${solutionsOpen ? "text-[#F97316]" : "text-white/70 hover:text-white"}`}
              ...
              {solutionsOpen && (
                <div className="absolute top-full left-1/2 -translate-x-1/2 mt-3 w-[640px] bg-[#0D0D0D] border border-white/8 shadow-2xl shadow-black/60 rounded-sm overflow-hidden">
                  {/* Mega menu header */}
                  <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between">
                    <div>
                      <p className="text-white font-700 text-sm uppercase" style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700 }}>
                        AI Marketing Solutions
                      </p>
                      <p className="text-white/40 text-xs mt-0.5" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                        Six tools built specifically for trade contractors
                      </p>
                    </div>
                    <Link
                      href="/solutions"
                      onClick={() => setSolutionsOpen(false)}
                      className="text-[#F97316] text-xs font-medium flex items-center gap-1 hover:gap-2 transition-all duration-200"
                      style={{ fontFamily: "'DM Sans', sans-serif" }}
                    >
                    ...
                    {solutions.map((sol) => {
                      ...
                      return (
                        <Link
                          ...
                          href={sol.href}
                          onClick={() => setSolutionsOpen(false)}
                          className="group flex items-start gap-3 p-4 bg-[#0D0D0D] hover:bg-[#F97316]/5 transition-colors duration-200"
                        >
                          ...
                          <div>
                            <p
                              className="text-white text-sm font-700 uppercase group-hover:text-[#F97316] transition-colors duration-200"
                  ...
                  <div className="px-6 py-3 bg-[#F97316]/5 border-t border-[#F97316]/10 flex items-center justify-between">
                    <p className="text-white/40 text-xs" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                      All solutions work together as one unified system
                    </p>
                    <a
                      href="/#contact"
                      onClick={() => setSolutionsOpen(false)}
                      className="bg-[#F97316] hover:bg-[#EA6C0A] text-black text-xs font-semibold px-4 py-2 rounded-sm transition-colors duration-200"
                      style={{ fontFamily: "'DM Sans', sans-serif" }}
                    >
                      Get Started
                    </a>
            ...
            <div className="relative" ref={learnRef}>
              <button
                onClick={() => { setLearnOpen(!learnOpen); setSolutionsOpen(false); }}
                className={`flex items-center gap-1.5 text-sm font-medium transition-colors duration-200 orange-underline ${learnOpen ? "text-[#F97316]" : "text-white/70 hover:text-white"}`}
                style={{ fontFamily: "'DM Sans', sans-serif" }}
              >
                Learn
              ...
              {learnOpen && (
                <div className="absolute top-full left-1/2 -translate-x-1/2 mt-3 w-72 bg-[#0D0D0D] border border-white/8 shadow-2xl shadow-black/60 rounded-sm overflow-hidden">
                  <div className="px-5 py-3 border-b border-white/5">
                    <p className="text-white font-700 text-sm uppercase flex items-center gap-2" style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 700 }}>
                      <BookOpen className="w-4 h-4 text-[#F97316]" /> The Contractor's Playbook
                    </p>
                    <p className="text-white/40 text-xs mt-0.5" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                      Practical guides for trade business owners
                    </p>
                  </div>
                  <div className="py-2">
                    {learnLinks.map((link) => (
                      <Link
                        ...
                        className="group flex flex-col px-5 py-3 hover:bg-[#F97316]/5 transition-colors duration-200"
                      >
                        <span
                          className="text-white/80 text-sm font-medium group-hover:text-[#F97316] transition-colors duration-200"
                          style={{ fontFamily: "'DM Sans', sans-serif" }}
                        >
                          {link.label}
                        </span>
                        <span className="text-white/35 text-xs mt-0.5" style={{ fontFamily: "'DM Sans', sans-serif" }}>
                          {link.desc}
                        </span>
                      </Link>
                    ))}
        ...
        <div className="md:hidden bg-[#0A0A0A]/98 backdrop-blur-md border-t border-white/5 max-h-[80vh] overflow-y-auto">
          <div className="container py-4 flex flex-col gap-1">
            ...
            <button
              ...
              className="flex items-center justify-between text-base font-medium text-white/80 hover:text-[#F97316] transition-colors py-3 border-b border-white/5"
              style={{ fontFamily: "'DM Sans', sans-serif" }}
            >
              Solutions
            ...
            {mobileSolutionsOpen && (
              <div className="pl-4 pb-2 space-y-1">
                {solutions.map((sol) => {
                  ...
                  return (
                    <Link
                      ...
                      className="flex items-center gap-3 py-2.5 text-white/65 hover:text-[#F97316] transition-colors"
                    >
                ...
                <Link
                  href="/solutions"
                  onClick={() => setMenuOpen(false)}
                  className="flex items-center gap-2 py-2 text-[#F97316] text-sm font-medium"
                  style={{ fontFamily: "'DM Sans', sans-serif" }}
                >
              ...
              <div className="pl-4 pb-2 space-y-1">
                {learnLinks.map((link) => (
                  <Link
                    ...
                    href={link.href}
                    onClick={() => setMenuOpen(false)}
                    className="block py-2.5 text-sm text-white/65 hover:text-[#F97316] transition-colors"
                    style={{ fontFamily: "'DM Sans', sans-serif" }}
                  >
                    {link.label}
                  </Link>
            ...
            <a
              href="/#contact"
              onClick={(e) => { scrollToSection(e, "#contact"); setMenuOpen(false); }}
